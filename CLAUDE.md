...
- **Spot arrays are `double[][]`** shaped `[spot][field]`, where column 0 is always a boolean-as-double "is this spot still good" flag written by each filter stage (`spotFilterWithMask`, `spotFilterSNR`, `spotFilterProminence`); later columns hold `x, y, snr, prominence` (see `columnHeaders` in `smFRETSpotFinder`). Filters append a diagnostic column (SNR or prominence value) rather than overwriting existing columns — check `columnHeaders`/array widths when adding a new filter stage.
