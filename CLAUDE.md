...
- **Background estimation is sigma-clipped normalized convolution.** `backgroundEstimate(image)` smooths the image and the trusted mask with the same Gaussian (`2 * spotMargin`) and divides one by the other, which fills the spots *and* the edges in one pass; then it drops pixels sitting more than `backgroundKappa` MAD-based standard deviations **above** that estimate and repeats, four rounds. The clipping is one-sided on purpose — contamination only ever brightens a pixel. There is no seeded second flavor: it converges in a fixed few rounds with nothing to carry over between frames, so `smFRETAnalyzer.backGroundEstimation()` estimates each frame independently. It replaced an iterated Gaussian inpaint that trusted `backgroundMask` completely; on the example long movie that inpaint drove two thirds of photobleached traces significantly below zero (median −23 photoelectrons), because a real PSF's wings reach past any masking radius a crowded field can afford and the estimator read the background high exactly where it was about to be subtracted. `backgroundKappa` defaults to 1.8, measured by asking where a photobleached trace actually sits; it is dataset dependent (the same movie wanted 2.5 at 488 spots and 1.8 at 553), and note the circularity that `spotFilterSNR` uses this estimator, so a better background admits more spots, which crowds the field, which wants a tighter clip. It travels to `smFRETAnalyzer` through the spot-finder JSON as `"background kappa"`, like `spotMargin` and `edgeMargin`; JSON written before this change lacks the key, and the reader falls back to the default. The estimate is converted back to the input's pixel type, so the stored background is still integer ADU.
